...
def test_register_world_adds_hook_and_writes_outbox(msg_store):
    hub = MessageHub(msg_store, channel=None)
    bus = EventBus()
    model_events = {"order.created": {"external": True}}

    hub.register_world("world-1", bus, model_events)
...
def test_unregister_world_removes_hook(msg_store):
    hub = MessageHub(msg_store, channel=None)
    bus = EventBus()
    model_events = {"order.created": {"external": True}}

    hub.register_world("world-1", bus, model_events)
    hub.unregister_world("world-1")
...
def test_non_external_event_does_not_write_outbox(msg_store):
    ...
    bus.publish("internal.tick", {"step": 1}, "src-1", "world")
...
def test_on_channel_message_writes_inbox(msg_store):
    ...
    hub.on_channel_message("notify.alert", {"level": "high"}, "ext-1", "world", "tgt-1")
...
def test_register_world_idempotent(msg_store):
    hub = MessageHub(msg_store, channel=None)
    bus = EventBus()
    model_events = {"order.created": {"external": True}}
    ...
    assert len(hub.registered_worlds()) == 1
...
def test_publish_does_not_trigger_pre_publish_hook(msg_store):
    ...
    hub.register_world("world-01", bus, {"order.created": {"external": True}})

    hub.publish("order.created", {"id": "123"}, "src-1", "world")
...
def test_publish_respects_target(msg_store):
    bus = EventBus()
    hub = MessageHub(msg_store, None)
    hub.register_world("world-01", bus, {"evt": {"external": True}})

    received_target = []
    received_other = []
    bus.register("inst-target", "world", "evt", lambda t, p, s: received_target.append(t))
    bus.register("inst-other", "world", "evt", lambda t, p, s: received_other.append(t))

    # target in EventBus is instance_id, not world_id
    hub.publish("evt", {"val": 1}, "src", "world", target="inst-target")
