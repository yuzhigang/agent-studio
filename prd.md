...
**`transitions`** 只定义状态机拓扑：from / to。不包含任何副作用，也不包含 trigger。
...
所有触发逻辑统一移入 `behaviors`（见 4.5）。`transitions` 只回答"能从哪到哪"，不回答"什么条件下走"。这样实现**单一职责**：想知道"某个状态变化会产生什么副作用"，只看 `behaviors`。

### 4.5 `behaviors`

`behaviors` 是 agent 所有**副作用的唯一归宿**，也是所有**触发逻辑的唯一归宿**。

统一触发框架的核心理念：**trigger 是通用框架，不只用于 transitions，也用于 behaviors。** 所有"触发 → 响应"走同一套系统。`transitions` 只定义状态图拓扑（从哪到哪），触发条件和响应动作全部在 `behaviors` 中声明。

涵盖以下来源的副作用：

1. 收到外部事件时的响应
2. 变量/属性变化时的响应（原 `stateEnter` 统一为 `valueChanged`）
3. 条件持续满足时的响应（condition trigger）
4. 时间到达时的响应（delay / interval / cron）
5. 状态迁移动作（`transition` action 类型）

#### Trigger 类型

| `type`         | 含义                                         | `name` 语义                  |
| -------------- | -------------------------------------------- | ---------------------------- |
| `event`        | 收到指定名称的事件（外部或自发）             | 事件名                       |
| `valueChanged` | 属性值变为目标值时触发（含 `state.current`） | 属性路径                     |
| `condition`    | 条件在窗口内持续满足时触发                   | 标签/标识                    |
| `delay`        | 延时一定时间后一次性触发                     | 标签/标识                    |
| `interval`     | 周期性触发                                   | 标签/标识                    |
| `cron`         | cron 表达式定时触发                          | 标签/标识                    |

**`stateEnter` 统一为 `valueChanged`：**

```json
// 旧写法（已废弃）
"trigger": { "type": "stateEnter", "state": "monitoring" }

// 新写法
"trigger": { "type": "valueChanged", "name": "state.current", "value": "monitoring" }
```

这样减少概念数量，state 变化与普通变量变化走同一套触发机制。
...
| -------------- | ------------------------ |
| `runScript`    | 在沙箱中执行脚本         |
| `triggerEvent` | 向事件总线发布事件       |
| `transition`   | 执行状态机迁移           |

**`transition` 是 behavior action 的一种：**

```json
"onStart": {
  "trigger": { "type": "event", "name": "start" },
  "actions": [
    { "type": "transition", "transition": "startMonitoring" }
  ]
}
```

行为触发后，actions 按顺序执行。`transition` action 修改 `state.current`，这会触发 `valueChanged` 监听器，形成级联。
...
"onEnterMonitoring": {
  "trigger": { "type": "valueChanged", "name": "state.current", "value": "monitoring" },
  "actions": [
    { "type": "runScript", "script": "this.variables.alertCount = 0" }
  ]
},
"onOverheat": {
  "trigger": {
    ...
    "window": { "type": "time-sliding", "duration": 1 }
  },
  "actions": [
    { "type": "transition", "transition": "tempOverThreshold" }
  ]
},
"onHeartbeat": {
  "trigger": { "type": "interval", "name": "heartbeat", "interval": 1000 },
  "actions": [
    { "type": "runScript", "script": "this.variables.count += 1" }
  ]
}
```
...
遥测流：通过实例层 bindings 绑定到 variables，
        由运行时引擎在变量更新后自动触发：
        派生属性重算 → valueChanged / condition trigger 评估
```
...
│   └─ 可能修改 variables → 触发 valueChanged / condition behaviors
│
├─ type = EVENT (behaviors 响应)
│   ├─ 匹配所有 trigger.type == "event" && name 相符的 behaviors
│   ├─ 按 priority / after 排序执行
│   ├─ 可能执行 transition action → state.current 变化
│   └─ state.current 变化触发 valueChanged 监听器 → 级联执行
│
└─ type = SCHEDULE (定时执行)
    ├─ 评估 condition（不满足则跳过）
    ├─ 执行 actions
...
| `transitions` 无 actions、无 trigger | 只有 from / to，触发条件和副作用全部进 behaviors   |
| `states` 无 actions                | 只有描述性元数据，副作用全部进 behaviors           |
| `behaviors` 统一所有 trigger       | transitions 和 stateEnter 的触发逻辑全部迁入       |
...
| 优先级 | 校验项                                                                      |
| ------ | --------------------------------------------------------------------------- |
| P0     | 引用完整性（behaviors 引用的 event/transition 必须存在）                    |
