# World-Instance-Scene 持久化架构设计文档

## 1. 背景与目标

当前 `World-Instance-Scene` 运行时架构（`EventBus`、`InstanceManager`、`SceneController`）完全基于内存运行。随着业务需求演进，World、Scene 和 Instance 都需要被持久化，支持：
...
1. **World 自包含**：每个 World 拥有独立的文件夹和 `runtime.db`，可整体迁移、备份、归档。
2. **零新增外部依赖**：默认使用 Python 内置 `sqlite3`，无需额外安装数据库。
3. **并发隔离**：不同 World 的 DB 文件完全隔离，彻底消除跨 World 锁竞争。
...
```yaml
worlds/
└── steel-plant-01/              # world_id
    ├── world.yaml             # World 元数据、全局配置
    ├── runtime.db               # SQLite 运行时数据库
    ├── scenes/                  # Scene 静态定义/模板（可选）
    └── resources/               # 外部资源、导入文件、附件
```

- `world.yaml`：静态源文件，包含 `world_id`、`name`、`description`、`config` 等全局配置。`WorldRegistry` 加载时从 `world.yaml` 读取，并镜像写入 `runtime.db` 的 `worlds` 表作为运行时缓存。
- `runtime.db`：统一存储 `scenes`（运行时状态）、`instances`、`event_log`、`world_state`。
- 整个文件夹可复制到任意环境直接加载运行。
...
class InstanceStore(ABC):
    ...
    def list_instances(self, world_id: str, scope: str | None = None, lifecycle_state: str | None = None) -> list[dict]: ...
    def delete_instance(self, world_id: str, instance_id: str, scope: str) -> bool: ...

class EventLogStore(ABC):
    def append(self, world_id: str, event_id: str, event_type: str, payload: dict, source: str, scope: str) -> None: ...
...
- **按 World 实例化**：`SQLiteStore(world_dir)` 只操作该目录下的 `runtime.db`。
- **单连接 + `threading.Lock()`**：同一个 World 内的并发写入由 Python 锁保护。
...
class PersistentEventBus:
    ...
    def publish(self, event_type, payload, source, scope, target=None, *, persist=True):
        ...
        if persist:
            self._store.append(
                world_id=..., event_id=generate_uuid(),
...
- `StateManager.checkpoint_world()` 在写入实例快照前，先确保最近一批事件已落盘。

## 5. WorldRegistry

`WorldRegistry` 是全局入口，负责发现、加载、卸载 World：
...
- `create_world()` 创建文件夹、`world.yaml`、`runtime.db`。
- `load_world()` 初始化 `SQLiteStore`（同时作为 World/Scene/Instance/EventLog Store）、`EventBusRegistry`（已存在于 `src/runtime/event_bus.py`，负责按 `world_id` 创建/销毁 `EventBus`）、`InstanceManager`、`SceneManager`、`StateManager`，返回运行时 bundle。
- `unload_world()` 关闭 SQLite 连接、清空内存实例、释放 EventBus。
...
- 遍历该 World 下所有 `active` 和 `completed` 实例。
- 在一个 SQLite 事务中原子写入：实例快照 + `lastEventId` 到 `world_state` 表。
- `world_state` 表以 `world_id` 为唯一主键，**仅保留最新的 checkpoint 元数据**，不保留历史版本。
- 若事务失败（如磁盘满、DB 锁定），`checkpoint_world()` 抛出 `RuntimeError`（或自定义 `PersistenceError`），且内存状态保持不变。
- 显式 checkpoint 由业务层在关键状态迁移后调用；同时 `StateManager` 内部维护一个 `loaded_worlds: set[str]`（受 `threading.Lock()` 保护），并启动一个轻量后台线程，每 30 秒对该集合的快照执行自动 checkpoint。
- 后台线程在对某个 World 执行 checkpoint 前，先尝试获取该 World 的 per-world 锁；若获取失败（说明正在 unload），则跳过。
- `unload_world()` 必须在释放资源前先将 `world_id` 从 `loaded_worlds` 中移除，再调用 `StateManager.shutdown()` 优雅停止后台线程，防止对正在卸载的 World 执行 auto-checkpoint。
- `MetricStore` 由 `WorldRegistry.load_world()` 在构造 `StateManager(metric_store=...)` 时注入，用于 metric 回填。`load_world()` 的伪代码如下：
  ```python
  store = SQLiteStore(world_dir)
  bus_reg = EventBusRegistry()
  im = InstanceManager(bus_reg, instance_store=store)
  metric_store = metric_store_factory(world_id) if metric_store_factory else None
...
- `checkpoint_scene(world_id, scene_id)`：原子快照所有 `scope == f"scene:{scene_id}"` 的实例（包括 CoW 副本和 local instances），同时保存 scene 自身的运行时状态到 `scenes` 表。
- `restore_scene(world_id, scene_id)`：从 DB 加载 scene 运行时状态和关联实例，重新注册到 EventBus，并执行 metric 回填与 Property Reconciliation。

### 6.2 Restore

1. 从 `InstanceStore` 读取 `active` / `completed` 实例快照到内存（`archived` 实例按需懒加载）。
2. 从 `world_state` 表读取 `lastEventId`。
...
- 新增 `transition_lifecycle(world_id, instance_id, new_state)`：
...
- 新增 `list_by_world(world_id)` 查询该 World 下所有运行时 Scene 状态。
...
class MetricStore(ABC):
    def write(self, world_id: str, instance_id: str, variable: str, value, timestamp: datetime) -> None: ...
...
- **World 之间**：完全并行，零锁竞争（每个 World 独立 `.db` 文件）。
- **World 内部**：`SQLiteStore` 单连接 + `threading.Lock()` 保护写入，WAL 模式保证读写不阻塞。
- **内存运行时**：保留现有的 `InstanceManager._lock` + `EventBus._lock` 机制。

## 11. 测试策略

1. `tests/runtime/stores/test_sqlite_store.py`：Store CRUD 单元测试。
2. `tests/runtime/test_world_registry.py`：WorldRegistry 完整生命周期测试。
