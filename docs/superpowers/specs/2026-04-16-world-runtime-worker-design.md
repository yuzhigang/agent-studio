...
当前 `agent-studio` 的 `WorldRegistry` 在同一个 Python 进程中加载所有 World，实例间通过独立的 `EventBus`、`InstanceManager` 和脚本 `SandboxExecutor` 实现逻辑隔离。然而：

- 一个 World 的脚本死循环、内存泄漏或 C 扩展崩溃可能拖垮整个进程。
- 数字孪生类应用需要长期稳定运行，与外部系统（RabbitMQ、OPC-UA 等）保持持久连接。
- 仿真类应用需要频繁启停，对启动速度和资源开销敏感。

本设计目标是：

1. 为需要长期稳定运行的 World 提供**进程级隔离**。
2. 为临时仿真类 World 保留**轻量进程内隔离**。
...
| 概念 | 定义 |
|---|---|
| **`WorldRuntime`** | `agent-studio` 的运行时进程，内部持有 `WorldRegistry`，负责加载 World、运行实例、消费外部事件。 |
| **Supervisor** | 可选的管理平面 + WebSocket 网关，负责管理多个 `WorldRuntime` 进程的生命周期和请求路由。 |

### 关键设计原则

- **一个 `WorldRuntime` 进程可以加载多个 World**（代码能力上）。
- 但在**生产隔离架构**下，我们**约束每个 `WorldRuntime` 进程只加载一个 World**，以获得最强的故障隔离。
- `WorldRuntime` 与 `Supervisor` 之间是**可选的管理关系**，不是运行时必需依赖。
...
### 3.1 `agent-studio run` — 单 World 隔离模式

```bash
# 独立运行（边缘部署）
agent-studio run --world-dir=worlds/steel-plant-01

# 独立运行 + 本地 WebSocket（供浏览器直连）
agent-studio run --world-dir=worlds/steel-plant-01 --ws-port=9001
...
- **进程模型**：当前进程**只加载一个 World**。
- **隔离级别**：**进程级隔离**。
- **适用场景**：生产环境、数字孪生、边缘部署。
- **参数约束**：`--world-dir` 在此子命令下只能出现**一次**。传入多次应报错。

### 3.2 `agent-studio run-inline` — 多 World 内联模式

```bash
agent-studio run-inline \
  --world-dir=worlds/factory-01 \
  --world-dir=worlds/factory-02 \
  --world-dir=worlds/factory-03
```

- **进程模型**：当前进程内加载多个 World。
- **隔离级别**：**线程/内存级隔离**（共享同一个 Python 进程）。
- **适用场景**：开发调试、集成测试、本地快速验证。
- **参数约束**：`--world-dir` 在此子命令下可出现**一次或多次**，每次指定一个 World 目录。
...
- **进程模型**：Supervisor 自身**不加载任何 World**，只负责管理子进程和网关路由。
- **适用场景**：集中管理多个 World 的生产环境。

---

## 4. 运行时排他锁

同一个 World **在同一时间只能被一个 `WorldRuntime` 进程加载**，防止多个进程并发写入同一个 `runtime.db`。

### 4.1 锁机制

- **文件锁**：在 World 目录下创建 `.lock` 文件，使用操作系统文件锁。
...
3. 若获取失败：
   ...
   - 抛出 `RuntimeError("World {id} is already loaded in process {pid}")`。
   - 若文件内容不可读，回退为通用错误 `RuntimeError("World {id} is already locked")`。
4. 若获取成功，立即将 `pid` 写入锁文件并 `flush`。
5. 在 World 卸载/进程退出时，释放锁并删除 `.lock` 文件。
...
- **推荐实现**：封装跨平台 `WorldLock` 工具类，优先使用 `fasteners` 或 `filelock` 等成熟库。
...
**启动流程：**
1. 客户端向 Supervisor 发送 HTTP/WebSocket 请求：`POST /api/worlds/{id}/start`
2. Supervisor 本地 `spawn` 子进程：
   ```bash
   agent-studio run --world-dir=worlds/{id} --supervisor-ws=ws://localhost:8001/workers
   ```
3. 子进程启动后，主动通过 WebSocket 连接 Supervisor，发送 `notify.runtimeOnline`。
4. Supervisor 将该 WebSocket 连接与 `{id}` 绑定，确认就绪。

**停止流程：**
1. 客户端向 Supervisor 发送 `POST /api/worlds/{id}/stop`
2. Supervisor 通过 WebSocket 向该子进程发送 `world.stop` JSON-RPC。
...
1. 边缘服务器由外部编排系统（systemd / Docker / k8s）执行：
   ```bash
   agent-studio run --world-dir=/opt/worlds/{id} --supervisor-ws=ws://central-supervisor:8001/workers
   ```
...
**停止流程（优雅）：**
1. 客户端向 Supervisor 发送 `POST /api/worlds/{id}/stop`
2. Supervisor 通过 WebSocket 发送 `world.stop` JSON-RPC。
3. 边缘子进程执行优雅停止并退出。

**停止流程（强制）：**
- Supervisor **无法跨网络发送 SIGTERM 或 SIGKILL**。
- 若子进程对 `world.stop` 无响应，Supervisor 只能：
  1. 标记该 World 为 "unreachable" 或 "dead"。
...
1. 开发者在本地直接执行：
   ```bash
   agent-studio run-inline --world-dir=worlds/factory-01 --world-dir=worlds/factory-02
   ```
2. 当前进程内依次加载多个 World，各自持有独立的 `EventBus` 和 `SQLiteStore`。
3. 无 Supervisor，控制通过本地 JSON-RPC over HTTP 或裸 Python API 完成。

**停止流程：**
1. 发送 SIGINT / SIGTERM 给当前进程。
2. 进程按加载顺序反向逐个停止 World（先 isolated scenes → 再 shared scenes → checkpoint → 释放锁 → unload）。
3. 进程退出。

---

## 6. Scene 与 World 的生命周期关系

| Scene 模式 | 生命周期绑定关系 | 启停方式 |
|---|---|---|
| **`shared`** | **随 World 一起启动、一起停止** | 自动，不可单独手动启停 |
| **`isolated`** | **依赖 World 处于运行状态，但可独立启停** | 用户/API 显式控制 |

### 6.1 `shared` Scene

- 是 World 的"实时视图"，直接引用 World 全局实例的内存状态。
- `agent-studio run` 加载 World 时，自动从 `scenes` 表恢复并启动所有 `shared` Scene。
- World 停止时，所有 `shared` Scene 必须先停止。
...
- 是独立的仿真上下文，基于 World 实例快照创建 CoW 副本。
- 启动 `isolated` Scene 的前提条件：对应 World 必须已被 `WorldRuntime` 加载。
- `isolated` Scene 的启停不影响 World 本身。
- 卸载 World 时，必须先尝试停止所有运行中的 `isolated` Scene。若某 Scene 拒绝停止（受 `force_stop_on_shutdown` 策略控制），则卸载返回 `False` 并中止；若策略为强制停止，则先停止 Scene 再执行卸载。
- `force_stop_on_shutdown` 默认值为 `false`（优先保护正在运行的仿真 Scene，避免误停）。可通过 CLI 参数 `--force-stop-on-shutdown=true|false` 覆盖，未来也可在 `world.yaml` 的 `config` 中声明持久化默认值。

---

## 7. 通信协议：统一 WebSocket + JSON-RPC 2.0

`WorldRuntime`（`agent-studio run`）对外暴露 WebSocket + JSON-RPC 2.0 接口，Supervisor 和浏览器客户端都可以连接。
...
| 方法 | 说明 |
|---|---|
| `world.stop` | 优雅停止当前 World，触发 checkpoint 后退出进程 |
| `world.checkpoint` | 立即执行一次全量 checkpoint |
| `world.getStatus` | 返回当前 World 健康状态和实例摘要 |
...
| 方法 | 方向 | 说明 |
|---|---|---|
| `notify.heartbeat` | Runtime → Supervisor/Client | 每 5 秒发送，包含 `world_id`、负载、运行状态 |
...
| `notify.runtimeOnline` | Runtime → Supervisor | 启动时发送，携带 `world_id` 和 `session_id` |
| `notify.runtimeOffline` | Runtime → Supervisor | 停止时发送 |
| `notify.sessionReset` | Supervisor → Client | 当检测到 runtime 重连时，通知客户端重新订阅 |

### 7.4 会话与重连

每次 `agent-studio run` 启动时生成一个 `session_id`（UUID），并在 `notify.runtimeOnline` 中携带。Supervisor 用 `session_id` 区分新旧连接。当同一个 `world_id` 出现新的 `runtimeOnline` 时，Supervisor 必须：
1. 关闭旧的 WebSocket 连接；
2. 用新连接替换 `world_id → WebSocket` 映射；
3. 向所有订阅该 World 的客户端广播 `notify.sessionReset`。
...
1. **解析 CLI 参数**：读取 `--world-dir`、`--supervisor-ws`（可选）、`--ws-port`（可选）。
2. **获取运行时排他锁**：在 World 目录下获取 `.lock` 文件锁。若已被占用，立即退出并报错。
3. **加载 World**：调用 `WorldRegistry.load_world(world_id)`。
4. **恢复状态**：`StateManager.restore_world()` → 加载实例快照 + 重放事件日志。
...
10. **WebSocket 断连处理**：若启动时指定了 `--supervisor-ws`，当检测到 WebSocket 意外断开且 15 秒内未能重连成功后，主动执行优雅停止（按 8.2 顺序）并退出进程，确保不会与新的 runtime 实例并发运行同一 World。

### 8.2 优雅停止顺序

1. 收到 `world.stop` RPC 或 **SIGTERM**。
2. 停止外部适配器。
3. 停止 `isolated` Scenes（受 `force_stop_on_shutdown` 策略控制，默认 `false`，可通过 `--force-stop-on-shutdown=true|false` 覆盖）。若策略为 `false` 且存在拒绝停止的 Scene，优雅停止流程应**中止**：`world.stop` RPC 返回错误码 `-32003`；SIGTERM 处理函数记录日志后正常返回（不退出进程），由调用方决定是否升级为 SIGKILL。
4. 停止 `shared` Scenes。
5. 禁用自动 checkpoint：将 `world_id` 从 `StateManager.loaded_worlds` 中移除，并获取 `StateManager` 内部的 per-world 内存锁（防止 auto-checkpoint 线程与显式 checkpoint/unload 竞态；注意这不是 `.lock` 文件锁）。
6. 执行最终 checkpoint：`StateManager.checkpoint_world()`。
7. 卸载 World：`WorldRegistry.unload_world(world_id)`。
...
1. 收到 SIGINT / SIGTERM。
2. 对每个已加载的 World，反向执行 8.2 的步骤 2-8。
3. 进程退出。

> **注意**：SIGTERM / SIGINT 作用于整个 `run-inline` 进程，会**无差别地停止所有已加载的 World**；`run-inline` 模式下不存在按 World 划分的信号级隔离。

---

### 8.4 World 内部事件驱动与行为执行模型

World 加载后，其业务逻辑的**核心动力源**是 `EventBus` 上的事件分发。

#### 8.4.1 事件总线与实例订阅

每个 World 拥有独立的 `EventBus`。当 `InstanceManager.create()` 创建一个 `Instance` 时，会自动解析该实例 `model.behaviors` 中所有 `trigger.type == "event"` 的行为，将实例注册为对应事件类型的订阅者：
...
`StateManager.restore_world()` 在加载实例后，会重放自上次 checkpoint 以来持久化的事件日志到 EventBus。这意味着：
...
- **反向注册管理**：接收来自边缘/本地 `agent-studio run` 的 WebSocket 连接，维护 `world_id → WebSocket` 映射。
- **请求路由**：把客户端的 `scene.start`、`world.checkpoint` 等请求转发到对应的 runtime。
...
| 优雅停止 | ✅ `world.stop` RPC | ✅ `world.stop` RPC |
...
- `WorldRegistry`、`InstanceManager`、`SceneManager`、`StateManager`、`SQLiteStore` 等核心代码**逻辑直接复用**，仅需在 `WorldRegistry.load_world()` 和 `unload_world()` 中增加 `.lock` 文件锁的获取与释放逻辑。
...
- **原因**：消除 `WorldWorker` 等过度包装的命名。`agent-studio run` 就是运行一个 World 的诚实表达；`run-inline` 是开发调试的轻量变体；`supervisor` 是纯管理平面。

### 决策 2：生产隔离 = 一个进程一个 World
- **原因**：现有代码本身支持一个进程内加载多个 World，但脚本级故障无法被沙箱完全拦截。强制一个进程只跑一个 World，是获得 OS 级故障隔离的最小代价方式。
...
### 决策 5：`shared` Scene 随 World 一起启停
- **原因**：`shared` Scene 直接引用 World 全局实例的内存状态，没有独立的生命周期意义。World 停止后，shared Scene 自然失效。

### 决策 6：运行时排他锁
- **原因**：多个 `WorldRuntime` 进程（或同一个 `run-inline` 和另一个 `run`）并发打开同一个 `runtime.db` 会导致数据损坏。文件锁是最简单且跨平台生效的防御手段。
...
### 决策 9：脚本错误静默捕获
- **原因**：`SandboxExecutor` 执行 behavior 脚本时可能因用户代码错误抛出异常。若将异常向上冒泡，会导致整个 EventBus 的 `publish` 中断，进而影响同 World 内其他实例的事件处理。静默捕获并忽略脚本错误，是以"隔离故障"换取"系统整体可用性"的务实选择。
...
- `src/runtime/locks/world_lock.py` 跨平台文件锁（Windows / Linux），含 Windows 双文件 workaround。
