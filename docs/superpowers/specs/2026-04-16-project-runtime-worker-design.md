...
2. 尝试以**非阻塞排他模式**获取文件锁。
3. 若获取失败：
   - 读取 `.lock` 文件中已有的 JSON 内容，提取 `runtime_type` 和 `pid`。
   - 抛出 `RuntimeError("Project {id} is already loaded as {runtime_type} in process {pid}")`。
   - 若文件内容不可读或已损坏，回退为通用错误 `RuntimeError("Project {id} is already locked")`。
4. 若获取成功，立即将本次启动的 `runtime_type` 和 `pid` 写入锁文件并 `flush`。
5. 在 Project 卸载/Worker 退出时，释放锁并删除 `.lock` 文件。
...
- **Unix**：`fcntl.flock` 是进程绑定的建议锁，进程崩溃后锁会自动释放。`.lock` 文件残留本身不会阻塞后续启动；启动时直接覆盖旧文件内容即可。
- **Windows**：`msvcrt.locking` 或 `portalocker` 的行为可能因库而异。如果进程崩溃后锁未释放，启动逻辑应：
  1. 读取锁文件中的 `pid`。
  2. 检查该 PID 是否仍在运行。
  3. 若 PID 已不存在，允许覆盖并获取锁；若 PID 仍在运行，则报错。
- **推荐实现**：在 `ProjectLock` 工具类中封装跨平台逻辑，优先使用 `fasteners` 或 `filelock` 等成熟库，它们已内置过期锁清理机制。
...
- 对于 `interactive` Project，调用 `ProjectRegistry.unload_project()` 时必须先尝试停止所有运行中的 `isolated` Scene。若某 Scene 拒绝停止（取决于 `force_stop_on_shutdown` 策略，见 8.3），则 `unload_project()` 返回 `False` 且继续保留 Project 加载状态；若策略为强制停止，则先停止 Scene 再执行卸载。
...
### 7.4 `interactive` Project 卸载顺序

`ProjectRegistry.unload_project(project_id)` 在释放 `InstanceManager` 和 `EventBus` 之前，必须完成以下步骤：

1. 尝试停止所有运行中的 `isolated` Scene（受 `force_stop_on_shutdown` 策略控制）。
2. 若存在拒绝停止的 Scene 且策略不允许强制停止，返回 `False` 并中止卸载。
3. 停止所有 `shared` Scene。
4. 执行最终 checkpoint：`StateManager.checkpoint_project()`。
5. 释放运行时排他锁并删除 `.lock` 文件。
6. 调用 `SQLiteStore.close()` 和 `EventBusRegistry.destroy()`，清空内存实例。
