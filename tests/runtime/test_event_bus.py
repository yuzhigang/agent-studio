...
def test_remove_pre_publish_hook():
    ...
    bus.register("inst-1", "world", "test.event", handler)
...
def test_scope_isolation_scene_vs_world():
    ...
    bus.register("ladle-002", "world", "ladleLoaded", lambda t, p, s: world_received.append(t))
...
def test_registry_get_or_create_and_destroy():
    ...
    assert bus1 is bus2
    bus3 = registry.get_or_create("world-b")
    assert bus3 is not bus1
    registry.destroy("world-a")
    bus4 = registry.get_or_create("world-a")
