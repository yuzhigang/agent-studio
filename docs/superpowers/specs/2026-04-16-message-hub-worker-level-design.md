...
1. 为每个 **Worker 进程**建立统一的 **MessageHub**，作为所有 World 的"事件控制平面"。
2. MessageHub 与 World 完全解耦：World 代码不感知 MessageHub 的存在。
...
- **Worker 级 MessageHub**：一个 Worker 进程内只存在一个 MessageHub 实例，所有加载的 World 共享它。无论是 `agent-studio run`（单 world）还是 `agent-studio run-inline`（多 world），MessageHub 都是单例。
- **World 零感知**：MessageHub 通过 `EventBus.pre_publish_hook` 拦截事件，不修改 world 代码、不污染 EventBus 的公共接口语义。
- **独立存储**：MessageHub 拥有独立的 `messagebox.db`，与 world 的 `runtime.db` 解耦。`inbox` / `outbox` 表不再包含 `world_id`。
- **订阅表路由**：World 加载时向 MessageHub 注册其订阅的事件列表。外部消息进入 inbox 后，InboxProcessor 查订阅表决定广播给哪些 world 的 EventBus。
...
### 4.2 World 注册到 MessageHub

Worker 加载 world 时执行注册：

```python
message_hub.register_world(
    world_id="factory-01",
    event_bus=bus,
...
2. **挂载 hook**：给该 world 的 `event_bus` 添加 `pre_publish_hook`。

卸载 world 时：

```python
message_hub.unregister_world("factory-01")
```

做反向清理：移除 hook、从订阅表删除该 world。
...
self._worlds: dict[str, tuple[EventBus, Callable]] = {}  # world_id -> (event_bus, hook_ref)
```
...
### 5.3 关键变化：去掉了 `world_id`

MessageHub 是 worker 级的，inbox / outbox 不再包含 `world_id`。路由由 InboxProcessor 根据订阅表完成。
...
### 5.5 与 world runtime.db 的边界

| 数据 | 存储位置 | 原因 |
|---|---|---|
| `inbox` / `outbox` | `messagebox.db` (worker 级) | 控制平面的持久化缓冲 |
| `event_log` | `runtime.db` (world 级) | 业务状态的事件溯源回放 |
| `instances` snapshot | `runtime.db` (world 级) | checkpoint 恢复 |

**设计保证**：`pre_publish_hook` 只写 outbox，不再写任何 world 的 `event_log`。`event_log` 保持纯业务语义。
...
2. 查订阅表得到目标 world 集合。
3. 对每个 world 调用其 `EventBus.publish()`。
4. **全部分发成功后**，`inbox_mark_processed(msg_id)`。

**边界**：
- 某个 world 分发失败不影响其他 world。
...
| 点 | 当前实现（per-world） | 新设计（per-worker） |
|---|---|---|
| Inbox 路由 | 发给单一 world | 查订阅表广播给多个 world |
| outbox 来源 | 单一 world | 所有 world |
| Channel 数量 | N world × 1 | 1 Worker × 1（MVP） |
| `send()` 调用方 | per-world OutboxProcessor | worker 级 OutboxProcessor |
...
```python
def run_world(world_dir, supervisor_ws=None, ...):
...
def _shutdown(signum, frame):
    ...
    if message_hub:
        ...
        for world_id in list(message_hub.registered_worlds()):
            message_hub.unregister_world(world_id)
```

**顺序**：先 stop MessageHub（停止 Processor 和 Channel），再 unregister world（移除 hook），最后 unload world。

### 7.5 动态 world 加载/卸载

支持运行时热加载：
- 加载：`message_hub.register_world(...)`
- 卸载：`message_hub.unregister_world(world_id)`
...
- `SQLiteMessageStore`：inbox/outbox 读写、无 `world_id` 的新 schema。
- `MessageHub`：
  - `register_world` / `unregister_world` 正确挂载/移除 hook。
  - `pre_publish_hook` 只把 `external=True` 事件写入 outbox。
  - 订阅表路由：单播、广播、空订阅。
- `InboxProcessor`：轮询、分发到多个 world EventBus、失败重试、空订阅直接标记 processed。
- `OutboxProcessor`：发送、重试、错误标记。
- `JsonRpcChannel` / `RabbitMQChannel`：保持不变，复用现有测试。

### 8.2 集成测试

- `run-inline` 双 world 场景：
  - World A 产生 `external=True` 事件 → outbox 有记录。
  - 模拟外部消息回环 → InboxProcessor 同时分发给 World A 和 World B。
...
- **原因**：`run-inline` 等多 world 场景下，per-world MessageHub 会导致资源冗余和无法统一策略。Worker 级单例更符合"控制平面"的定位。
...
- **原因**：在不修改 world 代码的前提下实现全局拦截，是最轻量、最彻底的解耦方式。

### 决策 4：订阅表维护在内存中
- **原因**：订阅信息来源于 world 加载时的静态 model 配置，变化频率极低。内存字典足够高效，无需落盘到 messagebox.db。

### 决策 5：Inbox 采用广播语义
- **原因**：一个外部事件（如 `order.created`）可能多个 world 都关心。广播是最自然的路由方式，各 world EventBus 再用自己的 `scope` / `target` 做二次过滤。
